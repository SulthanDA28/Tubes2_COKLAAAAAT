...
namespace BFS
{
    class cariruteBfs 
    {
        public static List<int[]> simpanruteBfs = new List<int[]>();
    }
    class Bfs
    {
            ...
            while (queue.Count > 0 && treasureCount > 0)
            {
                ...
                for (int i = 0; i < neighbors.Count; i++)
                {
                    if (!neighbors[i].getMarked())
                    {
                        ...
                        int[] buatbfs = new int[2];
                        buatbfs[0] = neighbors[i].getY();
                        buatbfs[1] = neighbors[i].getX();
                        cariruteBfs.simpanruteBfs.Add(buatbfs);
